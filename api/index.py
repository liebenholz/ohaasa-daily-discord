...
def handle_settings_command(interaction):
    ...
    if sub_name == "알림설정":
        ...
        if test["outcome"] == "permanent" and test.get("status_code") in (403, 404):
            return ephemeral(
                f"⚠️ 채널은 등록되었지만 테스트 메시지 전송에 실패했습니다.\n "
                f"봇에게 <#{channel_id}> 채널의 '채널 보기'/'메시지 보내기' 권한이 없는 것으로 보입니다.\n"
                f"채널 권한에서 봇 역할에 권한을 허용해주시면, 다시 등록하지 않아도 다음 발송부터 자동으로 반영됩니다.\n"
                f"-# 서버 ID: {guild_id}"
            )
        return ephemeral(
            f"✅ 알림 채널이 <#{channel_id}>(으)로 설정되었습니다. \n"
            f"테스트 메세지 전송은 다음 오류로 실패했습니다: {test.get('message') or test['outcome']}\n"
            f"-# 서버 ID: {guild_id}"
        )
